    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            else
            {
                ...
                if (args.FirstOrDefault() == "ReactorFor" || !cfg.PathToSystemBinary.IsNullOrWhitespace())
                {
                    var pathToExe = args.Skip(1).FirstOrDefault();
                    var url =  args.Skip(2).FirstOrDefault();

                    RxnProcessSupervisor.RunSupervisorReactor(pathToExe ?? cfg.PathToSystemBinary, cfg.Version, url,  args).Do(_ => { ctx = _; }).WaitR();
                ...
                if (args.FirstOrDefault() == "TestAgent"|| true)
                {
