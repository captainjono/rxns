...
namespace Rxns.WebApiNET5.NET5WebApiAdapters
{
    public class ReportsStatusApiController : Controller, IReportStatus
    {
        ...
        {
            if (error == null) return StatusCode(StatusCodes.Status500InternalServerError, "unknown");
#if DEBUG
            return StatusCode(StatusCodes.Status500InternalServerError, error.ToString());
#else
            var message = error.Message ?? "unknown";
            var isMultiline = message.IndexOf("\r\n", StringComparison.OrdinalIgnoreCase);
            return StatusCode(StatusCodes.Status500InternalServerError, message.Substring(0, isMultiline < 0 ? message.Length : isMultiline));
#endif
        }
