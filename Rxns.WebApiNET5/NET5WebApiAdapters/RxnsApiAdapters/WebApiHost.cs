    ...
    public class WebApiHost : ReportsStatus, IRxnHostReadyToRun
    {
        ...
        public IObservable<IRxnAppContext> Run(IAppContainer container = null)
        {
            return Observable.Create<IRxnAppContext>(o =>
            {
                ...
                {
                    // Used to call Environment.Exit(1969) here — that kills the entire
                    // hosting process, which is catastrophic when WebApiHost is reused
                    // across test methods or inside an IDE. Port-bind failures on 888
                    // (address-in-use) now propagate as an OnError instead, so the caller
                    // can decide whether to retry or fail the current operation only.
