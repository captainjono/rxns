...
namespace RxnsDemo.Micro.App.Windows
{
    ...
    public class WindowsFileSystemService : IFileSystemService
    {
        ...
        {
            return source.IsEqualsTo(destination);
        }
        ...
        public IEnumerable<IFileMeta> GetFiles(string path, string mask, bool searchRecursively = false)
        {
            ...
            return info.EnumerateFileSystemInfos(mask, searchRecursively ? SearchOption.AllDirectories : SearchOption.TopDirectoryOnly).Where(fsi => (fsi.Attributes & FileAttributes.Directory) != FileAttributes.Directory).Select(fi => new FileMeta(fi));
        }
        ...
        {
            return Observable.Defer(() => Observable.Create<Unit>(o =>
            {
