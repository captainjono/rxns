...
namespace Rxns.Health
{
    ...
    public class WindowsSystemInformationService : ISystemResourceService
    {
        ...
        public IObservable<float> CpuUsage { get { return _cpuUsage; } }
        public IObservable<float> MemoryUsage { get { return _memoryUsage; } }
        public IObservable<int> ThreadCount { get { return _threadCount; } }

        private readonly IObservable<float> _cpuUsage;
        private readonly IObservable<int> _threadCount;
        private readonly IObservable<float> _memoryUsage;
        ...
        public WindowsSystemInformationService(IOperationSystemServices systemServices, IScheduler scheduler = null)
        {
            ...
            _cpuUsage = Observable.Interval(TimeSpan.FromSeconds(1), DefaultScheduler).Select(_ => _cpuCounter.NextValue()).Publish().RefCount();
            _memoryUsage = Observable.Interval(TimeSpan.FromSeconds(1), DefaultScheduler).Select(_ => GetMemoryUtilisation()).Publish().RefCount();
            _threadCount = Observable.Interval(TimeSpan.FromSeconds(1), DefaultScheduler).Select(_ => Process.GetCurrentProcess().Threads.Count).Publish().RefCount();
        }
