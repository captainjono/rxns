...
namespace Autofac
{
    ...
    public class AutofacRxnDef : IRxnDef
    {
      ...
      //  public IContainer WrappedContainer;
        ...
        // Shared across every Configure() call so multiple UpdateWith()
        // configurators populate ONE registry — without this, each new
        // lifecycle had its own _emits, registered itself into the shared
        // builder, and Autofac's last-wins semantics on RegisterInstance
        // meant cc.Resolve<EmitsRegistry>() returned whichever lifecycle's
        // registry registered last (often empty), defeating ForEmits().
        private EmitsRegistry _sharedEmits;
        ...
        {
            if (_sharedEmits == null)
            {
                _sharedEmits = new EmitsRegistry();
                WrappedDef.RegisterInstance(_sharedEmits).As<EmitsRegistry>().SingleInstance();
            }
            return new AutofacRxnLifecycle(WrappedDef, _sharedEmits);
        }
    ...
    public class AutofacRxnLifecycle : IRxnLifecycle
    {
        ...
        // Shared registry — passed in by AutofacRxnDef.Configure() so multiple
        // lifecycle instances (one per UpdateWith call) all populate the SAME
        // EmitsRegistry. Without sharing, each Configure() created its own
        // registry and the last RegisterInstance won; ForEmits() resolved
        // whichever was registered last (often empty).
        private readonly EmitsRegistry _emits;

        public AutofacRxnLifecycle(ContainerBuilder cb)
            : this(cb, RegisterFreshRegistry(cb))
        {
        }

        public AutofacRxnLifecycle(ContainerBuilder cb, EmitsRegistry emits)
        {
            _cb = cb;
            _emits = emits;
        }

        private static EmitsRegistry RegisterFreshRegistry(ContainerBuilder cb)
        {
            // Back-compat for callers that construct the lifecycle directly
            // (without going through AutofacRxnDef.Configure). They get a
            // standalone registry — fine for tests / single-call setups.
            var r = new EmitsRegistry();
            cb.RegisterInstance(r).As<EmitsRegistry>().SingleInstance();
            return r;
        }
        ...
        public IRxnLifecycle EmitsAnyIn<T>() where T : IRxn
        {
            ...
            // EmitsAnyIn is a DI bulk-registration ("any of these IRxn types
            // might appear"), NOT a declaration that they should ride central.
            // Components that genuinely emit a type cross-process should call
            // .Emits<T>() explicitly.
            return this;
        }
