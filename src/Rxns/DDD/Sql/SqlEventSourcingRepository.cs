...
namespace Rxns.DDD.Sql
{
    ...
    public class SqlEventSourcingRepository<T> : SqlEventSourcingRepository, ITenantModelRepository<T> where T : IAggRoot, new()
    {
        ...
        private readonly BoundedContext.ITenantModelFactory<T> _dmFactory;
        ...
        public SqlEventSourcingRepository(BoundedContext.ITenantModelFactory<T> dmFactory, ITenantDatabaseFactory ormFactory, IResolveTypes type, IScheduler changesScheduler = null)
        {
        ...
        public IEnumerable<IDomainEvent> FromEventData(IEnumerable<EventData> data)
        {
            ...
            {
                var evt = e.Event.FromJson(e.Event.GetTypeFromJson(_type), null, jsonSettings);
    ...
    public class LegacySqlEventSourcingRepository<T> : SqlEventSourcingRepository, ITenantModelRepository<T>
        where T : IAggRoot, new()
    {
        ...
        public LegacySqlEventSourcingRepository(BoundedContext.ITenantModelFactory<T> dmFactory, ITenantDatabaseFactory ormFactory, IResolveTypes type, IScheduler changesScheduler = null)
        {
            _ormFactory = ormFactory;
            _dmFactory = dmFactory;
            _type = type;
