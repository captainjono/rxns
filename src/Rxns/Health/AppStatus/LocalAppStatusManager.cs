...
namespace Rxns.Health.AppStatus
{
    ...
    public class LocalAppStatusManager : ReportsStatus, IAppStatusManager, IRxnPublisher<IRxn>
    {
        ...
        // Phase 7n4 refactor: cmd-routing surface (FlushCommands) moved off
        // IAppStatusStore onto IAppCmdManager. This class needs the manager
        // for the route-flush call in UpdateSystemStatusWithMeta; everything
        // else still uses the store (log/cache/system-status).
        private readonly IAppCmdManager _appCmds;
        ...
        public LocalAppStatusManager(ISystemStatusStore systemStatus, IAppStatusStore appStatus, IAppCmdManager appCmds, IAppUpdateManager updates, IAppHeartBeatHandler[] heartBeatHandlers)
        {
    ...
    public class TenantOnlyStatusComparer : IEqualityComparer<KeyValuePair<SystemStatusEvent, object[]>>
    {
        ...
        {
            return (x.Key?.Tenant ?? string.Empty) == (y.Key?.Tenant ?? string.Empty);
        }

        public int GetHashCode(KeyValuePair<SystemStatusEvent, object[]> obj)
        {
            return (obj.Key?.Tenant ?? string.Empty).GetHashCode();
        }
    }
}
