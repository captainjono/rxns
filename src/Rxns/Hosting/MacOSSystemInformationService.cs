...
namespace Rxns.Hosting
{
    ...
    public class MacOSSystemInformationService : ISystemResourceService
    {
        ...
        public MacOSSystemInformationService(IOperationSystemServices systemServices, IScheduler scheduler = null)
        {
            ...
            var sharedUsage = GetUsage().Publish().RefCount();

            CpuUsage = sharedUsage.Select(info => info.CPU);
            MemoryUsage = sharedUsage.Select(info => info.MEM);

            AppUsage = sharedUsage
                .Buffer(15)
                .Select(samples =>
                {
                    ...
                    return new AppResourceInfo()
                    {
                        ...
                        CpuUsage = samples.Count > 0 ? (float)samples.Average(s => s.CPU) : 0f,
                        MemUsage = samples.Count > 0 ? (float)samples.Average(s => s.MEM) : 0f
                    };
                }).Publish().RefCount();
        }
