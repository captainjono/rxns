...
namespace Rxns.Hosting
{
    ...
    public class RxnApp : IRxnApp, IAppEvents
    {
        ...
        public static string GetIpAddress()
        {
            ...
            foreach (var network in networkInterfaces)
            {
                ...
                foreach (var address in properties.UnicastAddresses)
                {
                    ...
                    // On Windows prefer gateway-facing DHCP interfaces; on Linux
                    // GatewayAddresses is often empty even for the primary interface,
                    // so treat any non-loopback IPv4 address as a candidate.
                    var hasGateway = properties.GatewayAddresses.Count > 0;

                    try
                    {
                        if (hasGateway && address.IsDnsEligible && address.PrefixOrigin == PrefixOrigin.Dhcp)
                            return address.Address.ToString();

                        if (mostSuitableIp == null
                            || (hasGateway && !mostSuitableIp.IsDnsEligible)
                            || (address.IsDnsEligible && !mostSuitableIp.IsDnsEligible))
                            mostSuitableIp = address;
                    }
                    catch (Exception)
                    {
                        // ??= isn't available in C# 7.3 (netstandard2.0 default).
        ...
        public void Dispose()
        {
            // _onStart is not disposed here — FinallyR handlers in the Start() observable chain
            // fire during CompositeDisposable.Dispose() and need to call _onStart.OnNext().
            // Disposing _onStart before they fire causes ObjectDisposedException that crashes callers.
            // The subject is GC'd when the RxnApp instance is collected.
        }
    }
