...
namespace Rxns.Hosting
{
    public static class RxnApps
    {
        ...
        public static IObservable<Unit> CreateAppUpdate(string appName, string version, string appLocation, bool isLocal, IAppStatusCfg cfg, string appStatusUrl = "http://localhost:888", string[] exclusions = default(string[]), string knownSha = null)
        {
            if (appLocation.StartsWith("."))
                appLocation = Environment.CurrentDirectory;
            ...
            $"Version: {version}".LogDebug();
            $"Location: {appLocation}".LogDebug();
            if (!string.IsNullOrEmpty(knownSha)) $"Sha (pre-computed): {knownSha.Substring(0, Math.Min(12, knownSha.Length))}".LogDebug();
            ...
            return client.Upload(appName, version, appLocation, exclusions, knownSha);
        }
