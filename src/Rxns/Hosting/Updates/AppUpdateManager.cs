...
namespace Rxns.Hosting.Updates
{
    ...
    public class LocalAppUpdateServer : ReportsStatus, IAppUpdateManager, IRxnPublisher<IRxn>
    {
        ...
        public IObservable<IFileMeta> Upload(string systemName, string version, IFileMeta appUpdate, string knownSha = null)
        {
            if (!appUpdate.Name.ToLower().Contains("zip"))
                throw new ArgumentException("Update packages must be a zip file");

            OnInformation("Uploading version '{0}' for '{1}'{2}", version, systemName,
                string.IsNullOrEmpty(knownSha) ? "" : $" sha={knownSha.Substring(0, Math.Min(12, knownSha.Length))}");

            var update = appUpdate.Contents;

            return _client.CreateUpdate(systemName, version, update, knownSha)
                .FinallyR(() =>
                {
                    update.Dispose();
                })
        ...
        {
            OnInformation("Removing update '{0}@{1}'", systemName, version);
            return _client.RemoveUpdate(systemName, version);
        }
