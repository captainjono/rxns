...
namespace Rxns.Hosting.Updates
{
    ...
    public interface IUpdateStorageClient
    {
        ...
        /// <param name="knownSha">
        /// Optional pre-computed sha256 (lowercase hex). When supplied, content-addressed
        /// storage backends can dedup against existing content with the same sha — the
        /// stream may or may not be written depending on whether the content is already
        /// present. Callers in same-process scenarios (e.g. <c>FileSystemAppUpdateRepo</c>)
        /// trust this value; HTTP-backed clients re-compute server-side. Pass null to
        /// have the backend compute the sha itself.
        /// </param>
        /// <returns>If the update was created or not</returns>
        IObservable<bool> CreateUpdate(string systemName, string version, Stream update, string knownSha = null);
        ...
        /// <summary>
        /// Removes a specific update artifact for a system. No-op if the
    ...
    public interface IUpdateServiceClient
    {
        ...
        /// <param name="knownSha">
        /// Optional pre-computed sha256 (lowercase hex) of the bundle. Forwarded to
        /// <see cref="IUpdateStorageClient.CreateUpdate"/> so the storage backend can
        /// dedup byte-identical content uploaded under different versions. When null,
        /// the storage backend hashes the stream itself.
        /// </param>
        /// <returns></returns>
        IObservable<Unit> Upload(string system, string version, string sourceFolder, string[] exclusionsInSourceFolder, string knownSha = null);
        /// <summary>
