...

                {
                    ...
                    while (true)
                    {
                        ...
                        for (i = 0; i < charsRead; i++)
                            // Backward-compat framing: treat the legacy delimiter
                            // '' (Private Use Area — historical choice in
                            // UseDeserialiseCodec / NewtonSoftJsonCodec) as a frame
                            // boundary regardless of codec.Delimiter. Lets tapes written
                            // before the switch to newline-framed codecs (JsonLinesCodec
                            // etc.) still be read. Writes are unchanged — always emit
                            // exactly codec.Delimiter; '' is only accepted on read.
                            if (buffer[i] != _codec.Delimiter && buffer[i] != '')
